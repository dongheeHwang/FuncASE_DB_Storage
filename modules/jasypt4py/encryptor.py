...
from modules.jasypt4py.generator import PKCS12ParameterGenerator, RandomSaltGenerator, FixedSaltGenerator
...
class StandardPBEStringEncryptor(object):
    ...
    def pad(block_size, s):
        ...
        """
        pad_str = (s.decode('utf-8') if type(s) == bytes else s)+(block_size - len(s) % block_size) * chr(block_size - len(s) % block_size)
